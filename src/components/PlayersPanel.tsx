...
// ConnectionDetailModal is kept but disabled for now - will use later
...
export function PlayersPanel() {
  ...
  return (
    <div className="panel flex flex-col h-full overflow-hidden">
          ...
          return (
            ...
            >
              ...
              <button
                ...
                title={isScratched ? 'Scratched' : 'Click to filter starters by this player'}
              >
                <div className={`font-medium truncate flex items-center gap-1 ${isScratched ? 'line-through text-text-muted cursor-not-allowed' : isPicked ? colors.text : isFiltering ? 'text-accent' : 'text-primary group-hover:text-accent'}`}>
                  <span>{conn.name}</span>
      ...
      {/* Connection Detail Modal - Disabled for now, will use later
      {selectedConnection && (
